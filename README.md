...
- ⚡ **高速な並列処理**: 並列リクエストと自動レート制限により、大量の文献も数秒で取得可能です。
- 🔗 **引用範囲の圧縮**: `1,2,3` のような連続する番号を `1-3` と自動で圧縮整形します。
...
1. リポジトリをクローンするか、`PyRefPmid.py` ファイルを直接ダウンロードします。
...
> [!TIP]
> **スタンドアロン動作**: 本スクリプトは単一の Python ファイルで動作するため、ライブラリさえインストールされていれば、ファイルをプロジェクトフォルダにコピーするだけでどこでも使用できます。
