# PyRefPmid: Markdown PubMed Referencer (v1.1.0)
...
**注意:** 引用の形式は `--pmid-regex` コマンドライン引数で変更可能です。変更した場合は、原稿ファイル内の記述もそのパターンに合わせてください。
...
- **`--api-key API_KEY`**
  - **説明**: PubMed API キー。設定するとリクエスト制限が緩和されます。
  - **デフォルト**: 無

- **`--cache-file CACHE_FILE`**
  - **説明**: 論文情報のキャッシュに使用する JSON ファイルのパス。
  - **デフォルト**: OS の一時ディレクトリ内の `pyrefpmid_cache.json`。
...
- **キャッシュファイル**:
  - デフォルトでは、キャッシュファイルは OS の一時ディレクトリ (例: Windows の `C:\Users\<ユーザー名>\AppData\Local\Temp`、Linux の `/tmp` など) に `pyrefpmid_cache.json` という名前で作成されます。
